...
export function initHero() {
  const hero = $('.hero');
  if (!hero) return { intro: async () => {}, scroll: () => {} };
  ...
  const wins = $$('.float-win', hero);
  const mascot = $('.hero__mascot', hero);
  ...
  // The entrance is built right away (paused): its from-states are applied now, so when the loader
  // opens it reveals the first frame of the entrance — not the finished hero that then blinks away.
  // Wrappers (.cta-pop) are animated instead of the buttons, whose hover/magnetic transforms stay theirs.
  const tl = reduceMotion ? null : gsap.timeline({ paused: true });
  if (tl) {
    tl.from('.hero__holo', { scale: 1.2, opacity: 0, duration: 1.4, ease: 'power2.out' }, 0)
      ...
      .from('.hero__lead, .hero__cta > .cta-pop, .hero__chips > *', { y: 26, autoAlpha: 0, duration: 0.7, stagger: 0.07 }, 0.8)
    ...
    if (mascot) tl.from(mascot, { y: -90, autoAlpha: 0, duration: 0.9, ease: 'bounce.out' }, 2.1);
    tl.from('.hero__scroll', { y: 20, autoAlpha: 0, duration: 0.6 }, 1.8);
    tl.add(idle, '>');
  }

  /** scroll-away drift; created with the other ScrollTriggers once the page is measured from the top */
  const scroll = () => {
  ...
  async function intro() {
    ...
    await portrait.play();
    typeBubble();
    if (mascot) burstFrom(mascot, 'heart', 12, 0.7);
  }
